...
namespace PortalRSApi.Common
{
    ...
    {
        public XDocument RemoveNamespaces(XDocument oldXml)
        {
        ...
        /// </summary>
        public XDocument RemoveNamespaces(string oldXml)
        {
        ...
        private void Invoke(string methodName, bool encode)
        {
            ...
            req.Credentials = new NetworkCredential("portaleac", "e@acrio");
            req.Headers["SOAPAction"] = "\"http://tempuri.org/" + methodName + "\"";
            req.ContentType = "text/xml;charset=\"utf-8\"";
            req.Accept = "text/xml";
            req.Method = "POST";

            using (Stream stm = req.GetRequestStreamAsync().Result)
            {
                string postValues = "";
                foreach (var param in Params)
                {
                    if (encode) postValues += string.Format("<{0}>{1}</{0}>", WebUtility.HtmlEncode(param.Key), WebUtility.HtmlEncode(param.Value));
            ...
            using (StreamReader responseReader = new StreamReader(req.GetResponseAsync().Result.GetResponseStream()))
            {
